...
<div class="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    ...
    <form method="GET" action="{{ route('admin.categories') }}" class="w-full sm:w-auto flex items-center gap-2 max-w-md">
        <div class="relative flex-1 sm:w-64">
            <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari kategori..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition text-sm">
        </div>
        ...
        @if($search)
        <a href="{{ route('admin.categories') }}" class="px-3 py-2.5 border border-slate-200 text-slate-500 bg-slate-50 rounded-xl hover:bg-slate-100 text-sm font-medium transition flex items-center justify-center" title="Reset">
            Reset
        </a>
        @endif
    </form>
