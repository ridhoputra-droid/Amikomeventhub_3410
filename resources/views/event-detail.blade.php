        ...
        <div class="space-y-4">
            ...
            <div class="flex flex-wrap gap-6 text-slate-500 font-medium">
                <div class="flex items-center gap-2">
                    ...
                    <span>
                        @if($event->date instanceof \Carbon\Carbon || is_object($event->date))
                            {{ $event->date->translatedFormat('l, d M Y, H:i') }}
                        @else
                            {{ \Carbon\Carbon::parse($event->date)->translatedFormat('l, d M Y, H:i') }}
                        @endif
                    </span>
                </div>
            ...
            <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                <div>
                    ...
                    <p class="mt-4 text-indigo-100 flex items-center gap-2">
                        ...
                        @else
                            <span class="font-bold text-rose-300 uppercase tracking-wider">Tiket Habis!</span>
                        @endif
                    </p>
                </div>
                <div>
                    @if(($event->stock ?? 0) > 0)
                        <a href="{{ url('checkout/' . $event->id) }}"
                            class="inline-block px-10 py-5 bg-white text-indigo-600 rounded-2xl font-black text-xl hover:scale-105 hover:bg-slate-50 transition-all shadow-xl">
                            Pesan Sekarang
                        </a>
                    @else
                        <button disabled
                            class="inline-block px-10 py-5 bg-slate-400 text-slate-200 rounded-2xl font-black text-xl cursor-not-allowed shadow-xl">
                            Sold Out
                        </button>
                    @endif
                </div>
        ...
        <div class="space-y-4">
            ...
            <ul class="space-y-3 text-slate-500">
                <li class="flex items-start gap-2">
                    ...
                    <span>E-Ticket akan dikirimkan otomatis ke email Anda setelah pembayaran berhasil dikonfirmasi.</span>
                </li>
                <li class="flex items-start gap-2">
                    <svg class="w-5 h-5 text-green-500 mt-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                    <span>Tunjukkan QR Code E-Ticket Anda pada saat memasuki area acara untuk proses scan check-in.</span>
                </li>
                <li class="flex items-start gap-2 text-rose-500">
                    <svg class="w-5 h-5 text-rose-500 mt-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Harap diperhatikan bahwa tiket yang sah dan sudah dibeli tidak dapat ditukar ataupun di-refund.</span>
                </li>
            </ul>
        </div>
    </div>
